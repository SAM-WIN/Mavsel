...
public class Baseproj {
	...
	WebElement detlog=driver.findElement(By.xpath(det));
	set(detlog,info);
	}
	...
	public static void chosedrop(String drptxt,String val) {
		WebElement drp=driver.findElement(By.xpath(drptxt));
	...
	public static void txtbox(String boxval,String txtval) {
		WebElement box=driver.findElement(By.xpath(boxval));
		box.sendKeys(txtval);
	}
	public static void dataprint(String dat) {
		WebElement da=driver.findElement(By.xpath(dat));
	...
	public static void cal(String fval,String calval) {
		WebElement c= driver.findElement(By.xpath(fval));
		c.sendKeys(calval);
	}
	public static void searchhotel() throws InterruptedException {
		...
		for(int i=1;i<=8;i++)
		{
			...
			chosedrop("//select[@id='location']","London");
				...
				cal("//input[@id='datepick_in']","12/08/2023");
			}
			else if(i==6) {
				cal("//input[@id='datepick_out']","13/08/2023");
			}
			else if(i==7) {
				chosedrop("//select[@id='adult_room']","2 - Two");
			}
			else if (i==8) {
				chosedrop("//select[@id='child_room']","1 - One");
			}
		}
	}
	public static void bookhotel() {
		...
		{
			if(j==1) {
				txtbox("//input[@id='first_name']","Sam");
			}
			else if(j==2) {
				txtbox("//input[@id='last_name']","Curren");
			}
			else if(j==3) {
				txtbox("//textarea[@id='address']","14 street,laurel road,manchester");
			}
			else if(j==4) {
				txtbox("//input[@id='cc_num']","1234567891234567");
			}
			else if(j==5) {
